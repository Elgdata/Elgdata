...
public class MyDynamicResourceHandler implements DynamicResourceHandler {
	...
	static {
		HANDLERS.put("pureLuck", new AbstractDynamicResourceHandler() {
			public boolean isAllowed(String name, String meta, DeadboltHandler deadboltHandler,
	...
	public boolean checkPermission(String permissionValue, DeadboltHandler deadboltHandler,
			Http.Context ctx) {
		...
		if (subject != null) {
			...
			for (Iterator<? extends Permission> iterator = permissions.iterator(); !permissionOk
					&& iterator.hasNext();) {
